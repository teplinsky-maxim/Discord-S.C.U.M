...
from ..utils.exceptions import InvalidGuildInviteException
...
class Guild(object):
    ...
    def joinGuild(self, inviteCode, location, wait):
        ...
        if location in ("accept invite page", "join guild"):
            guildData = self.getInfoFromInviteCode(inviteCode, with_counts=True, with_expiration=True, fromJoinGuildNav=(location.lower() == "join guild"))
            if guildData.status_code != 200:
                raise InvalidGuildInviteException(f"getInfoFromInviteCode returned not 200 for {inviteCode}")
    ...
    def lookupSchool(self, email, allowMultipleGuilds, useVerificationCode):
        ...
        if useVerificationCode is not None:
